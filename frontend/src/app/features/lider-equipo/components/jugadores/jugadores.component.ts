...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule, ReactiveFormsModule, SafePipe, PaginationComponent, ConfirmDialogComponent, ToastComponent],
...
export class JugadoresComponent implements OnInit, OnDestroy {
  ...
  // UI: confirm/toast (sin alert/confirm nativos)
  showConfirmEliminar = signal(false);
  jugadorAEliminar = signal<Jugador | null>(null);
  ...
  onDocumentoSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (file.type !== 'application/pdf') {
        this.mostrarToast('warning', 'Archivo inválido', 'Solo se permiten archivos PDF.');
        input.value = '';
        return;
      }
      ...
      if (file.size > 5 * 1024 * 1024) {
        this.mostrarToast('warning', 'Archivo muy grande', 'El archivo no debe superar 5MB.');
        input.value = '';
        return;
      }
  ...
  verPDF(jugador: Jugador): void {
    if (!jugador.documento_pdf) {
      this.mostrarToast('info', 'Sin documento', 'Este jugador no tiene documento PDF cargado.');
      return;
    }
  ...
  guardarJugador(): void {
    if (this.jugadorForm.invalid) {
      ...
      const firstError = Object.keys(this.jugadorForm.controls).find(key => 
        this.jugadorForm.get(key)?.invalid
      );
      if (firstError) {
        setTimeout(() => {
      ...
      this.mostrarToast('warning', 'Formulario inválido', 'Por favor, completa todos los campos requeridos correctamente.');
      return;
    }

    // Validar que haya documento PDF si es nuevo jugador
    if (!this.isEditing() && !this.documentoFile) {
      this.mostrarToast('warning', 'Documento requerido', 'Debes cargar el documento de identidad en PDF.');
      return;
    }
    ...
    if (this.isEditing() && this.jugadorEditando()) {
      ...
      this.liderService.actualizarJugador(id, jugadorData).subscribe({
        next: () => {
          ...
          this.mostrarToast('success', 'Jugador actualizado', 'Los datos del jugador se actualizaron exitosamente.');
        },
        error: (error) => {
          console.error('Error al editar jugador:', error);
          this.mostrarToast('error', 'Error', error.error?.error || 'Error al editar jugador');
        }
      });
    } else {
      // ============================================
      ...
      this.liderService.crearJugador(jugadorData).subscribe({
        next: (response: any) => {
          ...
          if (!nuevoJugadorId) {
            this.mostrarToast('error', 'Error', 'No se obtuvo el ID del jugador.');
            return;
          }
          ...
          this.mostrarToast('success', 'Jugador creado', 'El jugador se creó exitosamente.');
        },
        error: (error) => {
          console.error('Error al crear jugador:', error);
          this.mostrarToast('error', 'Error', error.error?.error || 'Error al crear jugador');
        }
      });
    }
  }
  ...
  subirDocumentoJugador(idJugador: number): void {
    ...
    this.liderService.subirDocumentoJugador(idJugador, formData).subscribe({
      next: () => {
        // Feedback opcional: no spamear, pero sí dejarlo consistente
        this.mostrarToast('success', 'Documento subido', 'El documento se subió correctamente.');
      },
      error: (error) => {
        console.error('Error al subir documento:', error);
        this.mostrarToast('warning', 'Documento no subido', 'El jugador se creó, pero hubo un error al subir el documento.');
      }
    });
  }
  ...
  subirFotoJugador(idJugador: number): void {
    ...
    this.liderService.subirFotoJugador(idJugador, formData).subscribe({
      next: () => {
        this.mostrarToast('success', 'Foto subida', 'La foto se subió correctamente.');
      },
      error: (error) => {
        console.error('Error al subir foto:', error);
        this.mostrarToast('warning', 'Foto no subida', 'Hubo un error al subir la foto.');
      }
    });
  }

  eliminarJugador(jugador: Jugador): void {
    this.jugadorAEliminar.set(jugador);
    this.showConfirmEliminar.set(true);
  }

  confirmEliminarJugador(confirmed: boolean): void {
    const jugador = this.jugadorAEliminar();
    this.showConfirmEliminar.set(false);
    this.jugadorAEliminar.set(null);
    if (!confirmed || !jugador) return;
    ...
    this.liderService.eliminarJugador(jugador.id_jugador).subscribe({
      next: () => {
        ...
        this.mostrarToast('success', 'Jugador eliminado', 'El jugador se eliminó exitosamente.');
      },
      error: (error) => {
        console.error('Error al eliminar jugador:', error);
        this.mostrarToast('error', 'No se pudo eliminar', error.error?.error || 'No se puede eliminar un jugador que ha jugado partidos');
      }
    });
  }
    ...
    if (input.files && input.files.length > 0) {
      ...
      this.liderService.subirDocumentoJugador(jugador.id_jugador, formData).subscribe({
        ...
        error: (error) => {
          this.uploadingDocument.set(false);
          console.error('Error al subir documento:', error);
          this.mostrarToast('error', 'Error', 'No se pudo subir el documento.');
        }
      });
    }
  }
  ...
  onSearchInput(event: Event): void {
    const input = event.target as HTMLInputElement;
    this.buscarTexto.set(input.value);
