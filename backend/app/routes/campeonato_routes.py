...
campeonato_update_model = campeonato_ns.model('CampeonatoUpdate', {
    ...
    'logo_url': fields.String(description='URL del logo del campeonato')
})
...
campeonato_output_model = campeonato_ns.model('CampeonatoOutput', {
    ...
    'logo_url': fields.String(description='URL del logo del campeonato'),
    'codigo_inscripcion': fields.String(description='Código de inscripción'),
    'es_publico': fields.Boolean(description='Si el campeonato es público')
})
...
class CampeonatoDetail(Resource):
    ...
    @campeonato_ns.expect(campeonato_update_model, validate=False)  # Cambiado a False para permitir campos adicionales
    @campeonato_ns.marshal_with(campeonato_output_model, code=200, envelope='campeonato')
    @jwt_required()
    @role_required(['admin'])
    def put(self, id_campeonato):
        try:
            ...
            data = campeonato_ns.payload or request.get_json() or {}
            ...
            if 'logo_url' in data:
                campeonato.logo_url = data['logo_url']
            ...
            db.session.rollback()
            campeonato_ns.abort(400, error=f'Error en formato de fecha: {str(e)}')
        except Exception as e:
            db.session.rollback()
            error_msg = str(e)
            # Si es un error de validación de Flask-RESTX, extraer el mensaje
            if 'validation' in error_msg.lower() or 'expected' in error_msg.lower():
                campeonato_ns.abort(400, error=f'Error de validación: {error_msg}')
            campeonato_ns.abort(500, error=error_msg)
...
@campeonato_ns.route('/<int:id_campeonato>/logo')
@campeonato_ns.param('id_campeonato', 'ID del campeonato')
class CampeonatoLogo(Resource):
    @campeonato_ns.doc(description='Subir logo del campeonato', security='Bearer')
    @jwt_required()
    @role_required(['admin'])
    def post(self, id_campeonato):
        ...
        try:
            ...
            if 'logo' not in request.files:
                campeonato_ns.abort(400, error='No se envió archivo')
            
            file = request.files['logo']
            
            if file.filename == '':
                campeonato_ns.abort(400, error='No se seleccionó archivo')
            
            # Validar extensión
            allowed_extensions = {'png', 'jpg', 'jpeg', 'gif'}
            if not ('.' in file.filename and file.filename.rsplit('.', 1)[1].lower() in allowed_extensions):
                campeonato_ns.abort(400, error='Formato no permitido. Use PNG, JPG, JPEG o GIF')
            ...
            if file_size > 5 * 1024 * 1024:
                campeonato_ns.abort(400, error='El archivo es muy grande. Máximo 5MB')
